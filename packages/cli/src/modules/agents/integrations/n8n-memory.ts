...
export class N8nMemory implements BuiltMemory, BuiltObservationStore {
	...
	async getMessages(
		...
		const where: FindOptionsWhere<AgentMessageEntity> | FindOptionsWhere<AgentMessageEntity>[] =
			opts?.since
				? [
		...
		const entities = await this.messageRepository.find({
			where,
			order: opts?.limit !== undefined ? { createdAt: 'DESC' } : { createdAt: 'ASC' },
	...
	async getMessagesForScope(
		...
		if (scopeKind === 'thread') {
			return await this.getMessages(scopeId, opts);
		}

		// Resource scope: scopeId is encoded as `${agentId}:${resourceId}` by the
		// cli's `buildAgentResourceScopeResolver`. Decode it here so we can
		// partition by agent (via `execution_threads`) AND user (via
		// `agents_messages.resourceId`). The first colon separates the two —
		// resourceIds may themselves contain colons (e.g. chat-bridge IDs like
		// `chat:botId-userId`), so we split on the first only.
		if (scopeKind !== 'resource') {
			throw new UnexpectedError(
				`getMessagesForScope: scopeKind='${scopeKind}' is not implemented yet`,
			);
		}
		const colonIdx = scopeId.indexOf(':');
		if (colonIdx === -1) {
			throw new UnexpectedError(
				`getMessagesForScope: resource-scope scopeId must encode '<agentId>:<resourceId>' (got '${scopeId}')`,
			);
		}
		const agentId = scopeId.slice(0, colonIdx);
		const resourceId = scopeId.slice(colonIdx + 1);

		// Inner JOIN to `execution_threads` to filter by agent. `execution_threads`
		// shares its primary key with `agents_threads` / `agents_messages.threadId`.
		const qb = this.messageRepository
			.createQueryBuilder('m')
			.innerJoin('execution_threads', 'et', 'et.id = m.threadId')
			.where('et.agentId = :agentId', { agentId })
